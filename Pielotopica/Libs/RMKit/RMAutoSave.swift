...
/**
 ...
 ```
 */
public protocol RMAutoSavable:RMStorable{};public class RMAutoSave<T:RMAutoSavable>{public var value:T?{didSet{value.map{RMStorage.shared.store($0,for:_key)}}};public func manualSave(){value.map{RMStorage.shared.store($0,for:_key)}};public func reset() {RMStorage.shared.remove(with: _key)};public init(_ key:String){value=RMStorage.shared.get(for:RMStorage.Key(rawValue: key));_key=RMStorage.Key(rawValue:key)};private let _key:RMStorage.Key<T>}
