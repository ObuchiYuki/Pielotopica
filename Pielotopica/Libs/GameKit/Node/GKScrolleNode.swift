...
extension SKNode {
    @objc internal func touchBegan(from location: CGPoint) {}
    @objc internal func touchMoved(to location: CGPoint) {}
    @objc internal func touchEnded(at location: CGPoint) {}
}
...
public class GKScrollNode: SKSpriteNode {
    ...
    public override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        let endVector = CGPoint(x: location.x - _frameBeforeLocation.x, y: location.y - _frameBeforeLocation.y)
    ...
    @objc private func _updateDisplay(_ sender: CADisplayLink) {
        ...
        if _inertiaXRunning {
            ...
            _inertiaVector = CGPoint(x: vector.x - dx, y: vector.y)
            _scrollNode.setOffset(CGPoint(x: _scrollNode.contentOffset.x + vector.x, y: _scrollNode.contentOffset.y + vector.y))
        ...
        if _inertiaYRunning {
            ...
            _inertiaVector = CGPoint(x: vector.x, y: vector.y - dy)
            _scrollNode.setOffset(CGPoint(x: _scrollNode.contentOffset.x + vector.x, y: _scrollNode.contentOffset.y + vector.y))
...
private class _GKScrollNode: SKSpriteNode {
    ...
    private func _dragDidMove(to location: CGPoint) {
        ...
        setOffset(CGPoint(x: _startOffset.x + delta.x, y: _startOffset.y + delta.y))
    }
    
    private func _modifyOffset(_ offset: CGPoint) -> CGPoint {
        var _csize = CGSize(width: contentSize.width - size.width, height: contentSize.height - size.height)
    ...
    private func _updateIndicatorRect() {
        _verticalIndicator.size =     CGSize(width: 5, height: self.size.height)
        _verticalIndicator.position = CGPoint(x: self.size.width / 2, y: 0)
    }
...
private class GKScrollNodeScrolleIndicator: SKSpriteNode {
    ...
    func update() {
        ...
        self.delegate.resizeIndicator(_indicatorNode, to: CGSize(width: 5, height: indicatorH))
        _indicatorNode.position = CGPoint(x: 0, y: (self.size.height / 2) - indicatorF)
    }
    ...
    private func _setup() {
        ...
        self._indicatorNode.anchorPoint = CGPoint(x: 0.5, y: 1)
        self.addChild(_indicatorNode)
    }
}

extension CGPoint {
    fileprivate func into(_ min: CGPoint,_ max:CGPoint) -> CGPoint {
