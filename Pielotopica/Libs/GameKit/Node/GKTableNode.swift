...
public class GKTableNode: SKSpriteNode {
    ...
    public func dequeueReusableCell(for iditifier: String, at index: Int) -> GKTableNodeCell? {
        ...
        if let cell = _unrenderedCellsQueue.first(where: { $0._identifier == iditifier }) {
            guard let _rindex = _unrenderedCellsQueue.firstIndex(of: cell) else { return nil }
            _unrenderedCellsQueue.remove(at: _rindex)
            cell._index = index
            return cell
        }
    ...
    internal override func touchBegan(from location: CGPoint) {
    ...
    internal override func touchEnded(at location: CGPoint) {
        guard
        ...
        // delta
        let delta = CGPoint(x: touchStartLocation.x - location.x, y: touchStartLocation.y - location.y)
    ...
    private func _calcPositions(of cell: GKTableNodeCell) -> CGPoint {
        ...
        let uty:CGFloat = CGFloat(cell._index) * cellHeight - (self.size.height / 2) + (cellHeight / 2)
        let ut = CGPoint(x: 0, y: -uty)
